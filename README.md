...
**위험도 점수 계산 방식** (점수가 높을수록 위험, `supabase/functions/_shared/riskScore.ts`)

1. **전세가율 점수 (0~100)** — 전세가율을 구간별로 선형 환산합니다.
   - 80% 이하 0~30점 · 80~100% 30~60점 · 100~130% 60~90점 · 130% 이상 90~100점 (150%에서 100점 포화)
2. **아파트·빌라 가중합** — 실제 전세사기 피해가 집중되는 연립다세대(빌라)는 시세 파악이 어려워 위험이 더 크므로, 두 전세가율을 가중합해 하나의 "실효 전세가율"로 만듭니다: `빌라 전세가율 × 0.7 + 아파트 전세가율 × 0.3` (한쪽 데이터가 없는 달은 있는 쪽 비율만 사용)
3. **뉴스 언급 점수 (0~100)** — 월간 "{지역} 전세사기" 뉴스 언급 건수를 구간별로 선형 환산합니다.
   - 0~5건 0~20점 · 6~20건 20~50점 · 21건 이상 50~100점 (50건에서 100점 포화)
4. **종합 점수** = 전세가율 점수 × 0.7 + 뉴스 언급 점수 × 0.3 (소수 첫째 자리 반올림)
5. **등급** — 70점 이상 `위험` · 40~69점 `주의` · 40점 미만 `안전`
