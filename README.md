...
- **업로드 전 이미지를 최대 1200px로 다운스케일**: 카메라로 찍은 원본 해상도 사진을 그대로 보내면
  서버(magick-wasm)가 리사이즈 전에 원본 해상도로 먼저 디코드해야 해서 Edge Function CPU 시간
  제한(2초)을 초과했습니다(실측 `cpu_time_used` 최대 3872ms, 오류 546). 브라우저에서 업로드 전에
  1200px로 미리 축소하고 base64 인코딩도 청크 단위로 처리해 CPU 부하를 줄여 해결했습니다.
...
- **HUG 상습 채무불이행자 실명단 대조**: 마스킹 전 CLOVA OCR로 읽은 임대인 이름을
  ...
  Gemini에는 전달되지 않음)
- **종합 점수는 서버가 가중평균으로 계산**: Gemini는 권리관계·특약사항·건물상태 3개 항목만 채점하고,
  ...
  안심 시그널 맵과 동일하게 **높을수록 위험**합니다(2026-08-21 이전 분석 이력은 반대 방향으로
  계산돼 있어 결과 화면에 예전 기준임을 별도로 안내합니다). 산정 기준 전체는 앱 내 `/scoring`
  페이지에서 확인할 수 있습니다.
- **전세가율은 실거래가 기반 실측값**: 계약서 주소로 지역을 찾아(DB 함수 `match_region_by_address`)
  국토교통부 평균 매매가와 보증금을 비교합니다. 주소와 일치하는 지역이 없거나 해당 지역의 최근
  실거래가 데이터가 없으면 "데이터 없음"으로 표시하고 종합 점수 계산에서 제외한 뒤 남은 항목끼리
  가중치를 재정규화합니다.
...
- **최종 위험도 판정은 규칙 기반 키워드 매칭**: Robert Cialdini(1984, *Influence: The Psychology
  of Persuasion*)의 설득의 6원칙(희소성·권위·사회적 증거·상호성·호감·일관성) 기준으로 분류한
  카테고리별 키워드가 대화 텍스트에 등장하는지 매칭해 점수화하고, 등급(안전/주의/위험)을 매깁니다.
  Gemini 호출은 유지하되 최종 위험도(`riskLevel`)는 이 규칙 기반 결과를 우선합니다 — AI 주관적
  판단 대신 검증 가능한 심리학 이론 기반 규칙으로 신뢰도를 높였습니다.
...
- (관리자 전용) 배치 작업 상태판: 안심 시그널 맵·뉴스 갱신 배치가 정상 동작 중인지 보여주는
  운영용 카드로, RLS가 지정된 관리자 계정에만 조회를 허용해 일반 사용자에게는 아예 렌더링되지
  않습니다
...
| 배치 스크립트 | Node.js, cheerio, iconv-lite, undici (HUG 명단 동기화, GitHub Actions) |
...
`scripts/sync-hug-defaulters.mjs`(Node 스크립트, GitHub Actions 주 1회 실행)는 Edge Function이 아니라 별도 크롤러입니다. HUG 명단 페이지가 228페이지에 달해 Edge Function 실행시간 제한을 넘기 때문에 독립 스크립트로 분리했습니다. 페이지 요청 사이 500ms~1초 랜덤 딜레이, 연결 실패 시 재시도(최대 3회, 2~3초 대기), 20페이지마다 결과를 즉시 저장하는 체크포인트 방식을 적용해, 도중에 실패해도 처음부터 다시 돌지 않고 마지막으로 처리한 페이지부터 다음 실행이 이어서 진행합니다.
...
- **HUG 명단 동기화가 GitHub Actions(CI 환경)에서 IP 차단으로 실패할 수 있습니다**: HUG 사이트가
  짧은 시간에 반복 요청을 보내는 IP를 속도 제한/차단할 가능성이 있어, 자동 스케줄(주 1회)은 그대로
  유지하되 실패가 반복되면 로컬 PC(고정 IP)에서 `node scripts/sync-hug-defaulters.mjs`를 수동
  실행해 대체하는 운영 방식을 쓰고 있습니다.
- **마스킹된 이미지는 클라이언트로 반환되지 않습니다**: 개인정보가 제대로 가려졌는지 화면에서 직접
  눈으로 확인할 방법이 없습니다. 이는 실수가 아니라 마스킹 실패/원본 이미지가 어떤 경로로도 클라이언트에
  노출되지 않도록 하는 설계상 의도입니다.
